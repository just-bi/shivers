...
//      }
  ...
  clickHandler: function(e) {
    ...
    var toggleGroup = button.getToggleGroup(), handler;
    if (toggleGroup) {
      var depressedButton = this.getDepressedButtonInToggleGroup(toggleGroup);
      var eventData = {
        group: toggleGroup,
        oldButton: depressedButton,
        newButton: depressedButton === button ? null : button
      };
      if (me.fireEvent("beforeToggleGroupStateChanged", eventData) === false) {
        return;
      }
      if (depressedButton) {
        depressedButton.setDepressed(false);
        handler = depressedButton.conf.stateChangeHandler;
        if (iFun(handler)) {
          handler.call(depressedButton.conf.scope || this, depressedButton, "stateChanged", eventData);
        }
      }
      if (depressedButton !== button || this.groupCount(toggleGroup)!==1) {
        button.setDepressed(true);
        handler = button.conf.stateChangeHandler;
        if (iFun(handler)) {
          handler.call(button.conf.scope || this, button, "stateChanged", eventData);
        }
      }
      me.fireEvent("afterToggleGroupStateChanged", eventData);
    }
    else {
      button.press();
      handler = button.conf.pressedHandler;
      if (iFun(handler)) {
        handler.call(button.conf.scope || this, button, "pressed");
      }
  ...
  eachButton: function(callback, scope) {
    ...
    for (i = 0; i < n; i++) {
      node = childNodes[i];
      button = ToolbarButton.getInstance(node.id);
      ret = callback.call(scope, button, i);
      if (ret === false) return ret;
    }
    return true;
  },
  getButtonWithClass: function(name) {
    var b = null;
    this.eachButton(function(button){
      if (button.conf["class"] !== name) {
        return;
      }
      b = button;
      return false;
  ...
  displayButton: function(name, display){
    var button = this.getButtonWithClass(name);
    if (!button) {
      return;
    }
    Displayed.setDisplayed(button, display);
  },
  getButtonWithConf: function(conf){
    var b = null;
    this.eachButton(function(button, i){
      var p, c = button.conf;
      for (p in conf) {
        if (iUnd(c[p]) || c[p] !== conf[p]) {
          return true;
        }
      }
      b = button;
  ...
  displayGroup: function(name, display){
    this.eachButtonInGroup(function(button){
      Displayed.setDisplayed(button, display);
    }, this, name);
  },
  hideGroup: function(name){
    this.displayGroup(name, false);
  },
  showGroup: function(name){
    this.displayGroup(name, true);
  },
